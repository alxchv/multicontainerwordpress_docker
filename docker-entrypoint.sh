...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
	...
	if ! [ -e index.php -a -e wp-includes/version.php ]; then
	    ...
	    if [ ! -e wp-content/plugins/redis-cache ]; then
			...
			echo "Downloading https://downloads.wordpress.org/plugin/redis-cache.2.0.21.zip"
			curl -o redis-cache.2.0.21.zip -fsL "https://downloads.wordpress.org/plugin/redis-cache.2.0.21.zip"

			echo "Unzipping redis-cache.2.0.21.zip to /var/www/html/wp-content/plugins/"
			unzip -q redis-cache.2.0.21.zip -d /var/www/html/wp-content/plugins/
